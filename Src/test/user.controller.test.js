...
describe('User Controller Tests', () => {
    ...
    describe('createPaciente', () => {
        it('debería crear un paciente exitosamente con todos los campos', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ message: "Paciente creado exitosamente" });
        });
        ...
        it('debería crear un paciente con arrays vacíos cuando sintomas y alergias no son arrays', async () => {
            ...
            expect(Paciente).toHaveBeenCalledWith(expect.objectContaining({
                sintomas: [],
                alergias: [],
                diagnostico: '',
                medicamentoAtomar: ''
        ...
        it('debería manejar errores durante la creación', async () => {
            ...
            await expect(createPaciente(req, res)).rejects.toThrow('Error de base de datos');
        });
    });
    ...
    describe('updatePacienteById', () => {
        ...
        it('debería rechazar actualización si la cédula ya existe', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ message: "La cédula ya está en uso" });
        });
        ...
        it('debería rechazar actualización si el email ya existe', async () => {
            ...
            const pacienteExistente = { _id: 'otroPaciente', email: 'existente@email.com' };
            Paciente.findOne
        ...
        it('debería manejar errores del servidor', async () => {
            req.body = { nombres: 'Test' };
            Paciente.findOne.mockRejectedValue(new Error('Error de servidor'));
    ...
    describe('updatePacienteByIdSintomasAlergias', () => {
        ...
        it('debería manejar errores del servidor', async () => {
            req.body = { sintomas: ['test'] };
            Paciente.findByIdAndUpdate.mockRejectedValue(new Error('Error'));
    ...
    describe('updatePacienteByIdMedicamentosAtomar', () => {
        ...
        it('debería manejar errores del servidor', async () => {
            req.body = { medicamentoAtomar: ['test'] };
            Paciente.findByIdAndUpdate.mockRejectedValue(new Error('Error de servidor'));
    ...
    describe('getPacienteByIdDiagnostico', () => {
        ...
        it('debería lanzar error si el paciente no existe', async () => {
            ...
            await expect(getPacienteByIdDiagnostico(req, res)).rejects.toThrow("Paciente no encontrado");
        });

        it('debería manejar errores de base de datos', async () => {
            Paciente.findById.mockRejectedValue(new Error('Error de BD'));

            await expect(getPacienteByIdDiagnostico(req, res)).rejects.toThrow('Error de BD');
        });
    });
    ...
    describe('getPacienteById', () => {
        ...
        it('debería lanzar error si el paciente no existe', async () => {
            ...
            await expect(getPacienteById(req, res)).rejects.toThrow("Paciente no encontrado");
        });

        it('debería manejar errores de base de datos', async () => {
            Paciente.findById.mockRejectedValue(new Error('Error de conexión'));

            await expect(getPacienteById(req, res)).rejects.toThrow('Error de conexión');
        });
    });
    ...
    describe('deletePacienteById', () => {
        ...
        it('debería eliminar paciente exitosamente', async () => {
            ...
            expect(Paciente.findOneAndDelete).toHaveBeenCalledWith({ cedula: '1234567890' });
            expect(res.status).toHaveBeenCalledWith(200);
            expect(res.json).toHaveBeenCalledWith({ message: "Paciente eliminado exitosamente" });
        });
        ...
        it('debería manejar errores del servidor', async () => {
            Paciente.findOneAndDelete.mockRejectedValue(new Error('Error de eliminación'));

            await deletePacienteById(req, res);

            expect(res.status).toHaveBeenCalledWith(500);
            expect(res.json).toHaveBeenCalledWith({ message: 'Error de eliminación' });
        });
    });
});